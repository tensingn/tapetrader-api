...
function post_merch(description, type, condition, user){
	...
	const new_merch = {"description": description, "type": type, "condition": condition, "band": null, "user":user};
	return datastore.save({"key":key, "data":new_merch}).then(() => {return key});
}
...
function get_all_merch(req, paginate){
    if(paginate){
        ...
        return datastore.runQuery(q).then( (entities) => {
                results.items = entities[0].map(fromDatastore).filter( item => item.user === req.user.name);
                if(entities[1].moreResults !== Datastore.NO_MORE_RESULTS){
                    results.next = req.protocol + "://" + req.get("host") + "/merch" + "?cursor=" + entities[1].endCursor;
                }
                return results;
            });
    } else {
        ...
        return datastore.runQuery(q).then( (entities) => {
                return entities[0].map(fromDatastore).filer( item => item.user === req.user.name);
            });
    }
}
...
function patch_merch(id, description, type, condition, band, user){
    ...
    const merch = {"description": description, "type": type, "condition": condition, "band": band, "user":user};
    return datastore.save({"key":key, "data":merch}).then(() => {return key});
}
...
function post_band(name, genre, country){
    ...
    return datastore.save({"key":key, "data":new_band}).then(() => {return key});
}
...
function get_all_bands(req, paginate){
    if(paginate){
        var q = datastore.createQuery(BANDS).limit(5);
        var results = {};
        if(Object.keys(req.query).includes("cursor")){
            q = q.start(req.query.cursor);
        }
        return datastore.runQuery(q).then( (entities) => {
                ...
                if(entities[1].moreResults !== Datastore.NO_MORE_RESULTS){
                    results.next = req.protocol + "://" + req.get("host") + "/bands" + "?cursor=" + entities[1].endCursor;
                }
                return results;
            });
    } else {
...
app.get('/merch', checkJwt, function(req, res){
    const merch = get_all_merch(req, true)
	.then( (merch) => {
        merch.items.forEach( items => {             
                items["self"] = req.protocol + '://'  + req.get('host') + "/merch/" + items["id"];
            });
...
app.get('/merch/:id', checkJwt, function(req, res){
    ...
    const merch = get_merch(req.params.id).then( (merch) => {
        ...
        if(!merch[0]){
            ...
            res.status(403).send({'Error':'Forbidden. You do not have access to this merch item.'});
        } else if(!accepts){
            res.status(406).send({'Error':'Not Acceptable.'});
        } else if(accepts === 'application/json'){
            merch[0]["self"] = req.protocol + '://'  + req.get('host') + req.originalUrl;
...
app.post('/merch', checkJwt, function(req, res){
    ...
    if(req.body.description === null){
        res.status(400).send({'Error': 'The request object is missing a required attribute.'});
        return;
    }

    post_merch(req.body.description, req.body.type, req.body.condition, req.user.name).then( key => {
        const merch = get_merch(key.id).then( (merch) => {
                        merch[0]["self"] = req.protocol + '://'  + req.get('host') + req.originalUrl + '/' + key.id;
...
app.patch('/merch/:id', checkJwt, function(req, res){
    ...
    const prevmerch = get_merch(req.params.id).then( (prevmerch) => {
        ...
        if(typeof prevmerch[0] === 'undefined'){
            ...
            patch_merch(req.params.id, req.body.description || prevmerch[0].description, 
                        req.body.type || prevmerch[0].type, req.body.condition || prevmerch[0].condition, 
                        req.body.band || prevmerch[0].band, prevmerch[0].user).then( () => {
        ...
        if(typeof prevmerch[0] === 'undefined'){
            ...
            delete_merch(req.params.id).then( () => {
                    post_merch(req.body.description, req.body.type, req.body.condition, req.user.name).then( (key) => {
                        //get newly updated merch to send back to client
                        const merch = get_merch(key.id).then( (merch) => {
                            merch[0]["self"] = req.protocol + '://'  + req.get('host') + "/merch/" + key.id;
                            merch[0]["id"] = key.id;
...
app.delete('/merch/:id', checkJwt, function(req, res){
    ...
    if(req.get('Accept') !== 'application/json' &&
        req.get('Accept') !== '*/*'){
        res.status(406).send({"Error": "Content type not supported by the endpoint"});
        return;
    }
            ...
            if(merch[0].user !== req.user.name){
                ...
                if(merch[0].band !== null){
                    get_band(merch[0].band.id).then( (band) => {
                        ...
                        patch_band(merch[0].band.id, band[0].name, band[0].genre, band[0].country, band[0].merch).then( () => {
    ...
    const prevmerch = get_merch(req.params.merch_id).then( (prevmerch) => {
        ...
        const reqband = get_band(req.params.band_id).then((reqband) => {
            ...
            if(prevmerch[0].band !== null){
                res.status(403).send({"Error": "Forbidden. This merch item is already associated with a band."});
            }
            ...
            patch_merch(req.params.merch_id, prevmerch[0].description, prevmerch[0].type,
                    req.params.condition, {
                        "id": req.params.band_id, 
                        "name": reqband[0]["name"],
                        "self": req.protocol + '://'  + req.get('host') + "/bands/" + req.params.band_id
                    }, req.user.name).then(() => {
                        ...
                        patch_band(req.params.band_id, reqband[0]["name"], reqband[0]["genre"], reqband[0]["country"], 
                            reqband[0]["merch"]).then(() => {
                                res.status(204).end();
                                return;
                        });
            });
...
app.delete('/merch/:merch_id/bands/:band_id', checkJwt, function(req, res){
    ...
    const prevmerch = get_merch(req.params.merch_id).then( (prevmerch) => {
        ...
        const reqband = get_band(req.params.band_id).then((reqband) => {
            ...
            //(id, description, type, condition, band, user){
            patch_merch(req.params.merch_id, prevmerch[0].description, prevmerch[0].type,
                    req.params.condition, null, req.user.name).then(() => {
                            // reqband[0]["merch"].push({
                            //     "id": req.params.merch_id, 
                            //     "self": req.protocol + '://'  + req.get('host') + '/merch/' + req.params.merch_id}
                            // );
...
app.post('/bands', function(req, res){
    ...
    post_band(req.body.name, req.body.genre, req.body.country).then( key => {
        const band = get_band(key.id).then( (band) => {
                        band[0]["self"] = req.protocol + '://'  + req.get('host') + req.originalUrl + '/' + key.id;
...
app.get('/bands/:id', function(req, res){
    const band = get_band(req.params.id).then( (band) => {
            if(!band[0]){
                ...
                band[0]["self"] = req.protocol + '://'  + req.get('host') + req.originalUrl;
...
app.get('/bands', function(req, res){
    const merch = get_all_bands(req, true)
    .then( (bands) => {
        bands.items.forEach( items => {             
                items["self"] = req.protocol + '://'  + req.get('host') + "/bands/" + items["id"];
            });
...
app.patch('/bands/:id', function(req, res){
    ...
    const prevband = get_band(req.params.id).then( (prevband) => {
        ...
        patch_band(req.params.id, req.body.name || prevband[0].name, 
                    req.body.genre || prevband[0].genre, req.body.country || prevband[0].country,
                    prevband[0].merch).then( () => {
        ...
        delete_band(req.params.id).then( () => {
                post_band(req.body.name, req.body.genre, req.body.country).then( (key) => {
                    ...
                    const band = get_band(key.id).then( (band) => {
                        ...
                        band[0]["self"] = req.protocol + '://'  + req.get('host') + "/bands/" + key.id;
                        band[0]["id"] = key.id;                        
                        res.status(200).send(band[0]);
                        return;
                    });
        ...
        if(typeof band[0] !== 'undefined'){
            for(var i = 0; i < band[0].merch.length; i++){
                ...
                get_merch(id).then( (merch) => {
                    patch_merch(id, merch[0].description, 
                        merch[0].type, merch[0].condition, null, merch[0].user).then( () => {});
                }); 
            }
        ...
        if (error){
            ...
            post_users(body._id, body.email).then( key => {
                    const user = get_user(key.id).then( (user) => {
                        user[0]["self"] = req.protocol + '://'  + req.get('host') + req.originalUrl + '/' + key.id;
